...
int main(int argc, char* argv[])
{
	...
	for (i = 0; i < MAX_EMPLOYEES; i++) {
		// ADD CODE
    emp *theEmployee;
    theEmployee = malloc (empArr[i] *sizeof(emp *)); //allocate memory for the employee
    if(theEmployee == NULL){
  ...
  emp *foundEmployee = findEmployee(empArr,sizeof(empArr) / sizeof(empArr **),"Carp");

	// if found print the record
  if(foundEmployee!=NULL){
    printf("%s     %s\n",theEmployee->firstName, theEmployee->familyName);
    printf("salary=%d     years of service=%f\n",theEmployee->salary, theEmployee->yearsWithCompany);
  }
  else{
    printf("ERROR: EMPLOYEE WAS NOT LOCATED!\n");

  }


	// add code to search for employee against the family name "King"
  emp *foundEmployee = findEmployee(empArr,sizeof(empArr) / sizeof(empArr **),"King");
