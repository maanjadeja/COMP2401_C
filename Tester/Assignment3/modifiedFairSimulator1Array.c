...
Guest *getDynamicallyAllocatedArrayForGuests(int);    //Function to dynamically allocate a Guest pointer varaible according to the size given in the parameters and return that Guest pointer
void freeTheDynamicallyAllocatedGuestArray(Guest*);   //Function to free the dynamically allocated Guest array we made in the getDynamicallyAllocatedArrayForGuests function
...
int main(int argc, char **argv){
    ...
    Guest *dynamicallyAllocatedGuestArray = getDynamicallyAllocatedArrayForGuests(MAX_GUESTS); //Calling getDynamicallyAllocatedArrayForGuests function to create dynamically allocated array
    for(int i=0; i<MAX_GUESTS; i++){

      //Initializing all values of the guest data to 0
      dynamicallyAllocatedGuestArray[i].age=0;
      dynamicallyAllocatedGuestArray[i].heightInCm=0;
      dynamicallyAllocatedGuestArray[i].totalMoneyOnGuest=0.00;
      dynamicallyAllocatedGuestArray[i].totalNumberOfTicketsOnGuest=0;
      dynamicallyAllocatedGuestArray[i].allDayRidePassOnGuest=0;
      dynamicallyAllocatedGuestArray[i].totalRidesGuestRode=0;


      int ageValueOfTheGuest = calculateAgeValueOfGuest(i);                     //Calling calculateAgeValueOfGuest function to find age of the guest
      dynamicallyAllocatedGuestArray[i].age = ageValueOfTheGuest;               //Setting age of the guest to calculated age
      calculateMoneyOnGuest(&dynamicallyAllocatedGuestArray[i]);                //Calling calculateMoneyOnGuest function to set the money of the guest
      calculateHeightOnGuest(&dynamicallyAllocatedGuestArray[i]);               //Calling the calculateHeightOnGuest function to set the height of the guest

      if(admitGuestToFair(&funFair,&dynamicallyAllocatedGuestArray[i]) == 0){   //Handling the case where the guest could not be admitted to the fair and making sure they do not purchase tickets or all day ride passes
        ...
        if(coinToss==0){  //Handling case where guest will try to purchase all day ride pass
          ...
          if(purchaseAllDayRidePassForGuest(&funFair,&dynamicallyAllocatedGuestArray[i])==0){ //Handling the case where guest could not purchase an all day ride pass

            int currentNumberOfTicketsGuestWantsToPurchase = NUMBER_OF_TICKETS_WHEN_GUEST_CANT_BUY_ALL_DAY_RIDE_PASS;


            if(purchaseRideTicketsForGuest(&funFair,&dynamicallyAllocatedGuestArray[i],currentNumberOfTicketsGuestWantsToPurchase)==0){


              int theUpdatedNumberOfTicketsGuestWantsToPurchase  = currentNumberOfTicketsGuestWantsToPurchase;

              while(purchaseRideTicketsForGuest(&funFair,&dynamicallyAllocatedGuestArray[i],theUpdatedNumberOfTicketsGuestWantsToPurchase)==0){
          ...
          if(purchaseRideTicketsForGuest(&funFair,&dynamicallyAllocatedGuestArray[i],theNumberOfTicketsTheGuestWantsToPurchase)==0){  //Handling case where guest could not purchase the calculated value of ride tickets

            int updatedNumberOfTicketsGuestWantsToPurchase=theNumberOfTicketsTheGuestWantsToPurchase;
            while(purchaseRideTicketsForGuest(&funFair,&dynamicallyAllocatedGuestArray[i],updatedNumberOfTicketsGuestWantsToPurchase)==0){
    ...
    for(int j=0; j<MAX_GUESTS; j++){
      ...
      while(repeatingRideProcess20Times<20){  //Variable to make sure that the guests attempt to go on at exactly 20 rides

        if(dynamicallyAllocatedGuestArray[j].age<=12){  //Handling the case where the guest has age less than or equal to 12
          ...
          guestAttemptsToGoOnRide(&dynamicallyAllocatedGuestArray[j],&funFair.allRides[randomIndexValueForFirstFiveRidesInRideArray]);   //Calling guestAttemptsToGoOnRide function to make guest attemp to go on the specific ride
          ...
          guestAttemptsToGoOnRide(&dynamicallyAllocatedGuestArray[j],&funFair.allRides[randomIndexValueForLastFiveRidesInRideArray]);   //Calling guestAttemptsToGoOnRide function to make guest attemp to go on the specific ride
    ...
    for(int k=0; k<MAX_GUESTS; k++){
      ...
      printf("Guest #%3d,  Age:%4d,  Height:  %dcm,  Money:  $%6.2f,  Tickets:",k, dynamicallyAllocatedGuestArray[k].age, dynamicallyAllocatedGuestArray[k].heightInCm, dynamicallyAllocatedGuestArray[k].totalMoneyOnGuest);
      if(dynamicallyAllocatedGuestArray[k].allDayRidePassOnGuest==1){
        printf("PASS,");
      }
      else{

        printf("%4d,",dynamicallyAllocatedGuestArray[k].totalNumberOfTicketsOnGuest);

      }

      printf("  Rides:%4d\n",dynamicallyAllocatedGuestArray[k].totalRidesGuestRode);
    ...
    freeTheDynamicallyAllocatedGuestArray(dynamicallyAllocatedGuestArray);  //Making sure we free the dynamically allocated array we dynamicallyAllocatedGuestArray allocated earlier
...
int calculateAgeValueOfGuest(int indexValueInGuestArray){
  ...
  int purchaseRideTicketsForGuest(Fair* theFair,Guest* theGuest,int numberOfTicketsRequestedToBePurchased){
    ...
    if(currentNumberOfTicketsAvailable < numberOfTicketsRequestedToBePurchased){  //Handling the case where there are not enough tickets to sell to the guest
      ...
      if(totalPriceOfTicketPurchase <= theGuest->totalMoneyOnGuest){  //Handling the case where the guest can afford the total price of tickets they requested to purchase
        ...
        theGuest->totalNumberOfTicketsOnGuest= theGuest->totalNumberOfTicketsOnGuest + numberOfTicketsRequestedToBePurchased;
  ...
  Guest *getDynamicallyAllocatedArrayForGuests(int sizeOfTheDynamicallyAllocatedArray){

    Guest *guestPointer = (Guest *) malloc(sizeOfTheDynamicallyAllocatedArray * sizeof(Guest));

    if(guestPointer==NULL){ //Handling the case where the memory allocation was unsuccessful

      exit(0);

    }
    for(int i=0; i<sizeOfTheDynamicallyAllocatedArray; i++){

      Guest newGuest;
      guestPointer[i]=newGuest; //Adding guests to the memory allocated array
  ...
  void freeTheDynamicallyAllocatedGuestArray(Guest* theNewDynamicallyAllocatedArray){

    free(theNewDynamicallyAllocatedArray);  //Using free function to make sure we free the dynamically allocated array
