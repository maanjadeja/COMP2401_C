...
Guest *allocateMemoryForIndividualGuest(Guest*);
int freeDynamicallyALlocatedGuest(Guest*);
...
int main(int argc, char **argv){
    ...
    for(int a=0; a<MAX_GUESTS; a++){
      allocateMemoryForIndividualGuest(&allGuests[a]);
    }
      ...
      if(admitGuestToFair(&funFair,&allGuests[i]) == 0){
        ...
        if(i<=MAX_GUESTS/2){
          ...
          if(purchaseAllDayRidePassForGuest(&funFair,&allGuests[i])==0){
            ...
            purchaseRideTicketsForGuest(&funFair,&allGuests[i],NUMBER_OF_TICKETS_WHEN_GUEST_CANT_BUY_ALL_DAY_RIDE_PASS);
          ...
          if(purchaseRideTicketsForGuest(&funFair,&allGuests[i],theNumberOfTicketsTheGuestWantsToPurchase)==0){
            ...
            while(purchaseRideTicketsForGuest(&funFair,&allGuests[i],updatedNumberOfTicketsGuestWantsToPurchase)==0){
              ...
              if(updatedNumberOfTicketsGuestWantsToPurchase==5){
    ...
    for(int m=0; m<MAX_GUESTS; m++){

      freeDynamicallyALlocatedGuest(&allGuests[m]);
...
int calculateAgeValueOfGuest(int indexValueInGuestArray){
  ...
  Guest *allocateMemoryForIndividualGuest(Guest* theGuest){

    // *student = (StudentType *) malloc(sizeof(StudentType));

    Guest *guestPointer = (Guest *) malloc (sizeof(Guest));
  ...
  int freeDynamicallyALlocatedGuest(Guest* theGuest){
