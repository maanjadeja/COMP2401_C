...
void *handleIncomingRequests(void *x) {
	...
	while(1){
		...
		int indexOfPIDValue=0;
		for(int j=theIndexValueOfTheComma+1; j<30; j++){

			thePIDValue[indexOfPIDValue] = buffer[j];
			indexOfPIDValue++;
		...
		printf("THE PID VALUE %s\n",thePIDValue);

		unsigned int theUnsignedIntPIDValue=atoi(thePIDValue);

		 //int i = atoi(myarray);

		printf("THE UNSIGNED INT PID VALUE %u\n",theUnsignedIntPIDValue);
		...
		switch(incomingRequest){	//We are supposed to look at the first value in bytesRcv OR buffer?
			...
			case ADMIT:
				...
				if(theFair->numGuests == MAX_GUESTS){
					...
					theFair->guestIDs[theFair->numGuests] = theUnsignedIntPIDValue;
				...
				//	char *allRideAndTickets="";
					...
					for(int j=0; j<NUM_RIDES; j++){
						...
						char ch = '-';
						strncat(rideNameAndRideTickets, &ch,1);
					...
					printf("SERVER: Sending \"%s\" to client\n", rideNameAndRideTickets);
					send(clientSocket, rideNameAndRideTickets, strlen(rideNameAndRideTickets), 0);
