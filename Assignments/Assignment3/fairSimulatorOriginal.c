...
int main(int argc, char **argv){
    ...
    for(int i=0; i<MAX_GUESTS; i++){
      ...
      allGuests[i].allDayRidePassOnGuest=0; //INITIALLY SETTING THE ALL DAY RIDE PASS VALUE FOR EACH GUEST TO 0 BECAUSE THEY HAVE NOT YET PURCHASED ONE, MAKE SURE YOU ASK ABOUT THIS TO THE TA
        ...
        if(i<MAX_GUESTS/2){
          ...
          if((purchaseRideTicketsForGuest(&funFair,&allGuests[i],numberOfTicketsGuestWantsToPurchase)) == 0){
            ...
            while((purchaseRideTicketsForGuest(&funFair,&allGuests[i],updatedNumberOfTicketsGuestWantsToPurchase)) == 0){ //THIS IS CAUSING THE FUNCTION TO GO INTO AN INFINITE LOOP??????
              ...
              //printf("THE PURCHASE OF BUYING %d TICKETS WAS NOT SUCCESSFUL BECAUSE THE GUEST HAS $.2f AND THE TOTAL PRICE OF BUYING %d TICKETS WAS GOING TO BE $.2f\n",updatedNumberOfTicketsGuestWantsToPurchase,allGuests[i].totalMoneyOnGuest,updatedNumberOfTicketsGuestWantsToPurchase,theFinalPrice);
    ...
    for(int j=0; j<MAX_GUESTS; j++){
      ...
      //allGuests[j].allDayRidePassOnGuest=1;
      ...
      /*allGuests[j].totalNumberOfTicketsOnGuest=0;


      if(allGuests[j].age <=12){

        if(allGuests[j].allDayRidePassOnGuest==1){

          printf("\n");
          printf("TESTING GUEST TO ATTEMPT TO GO ON RIDE WHEN THEIR AGE IS LOWER THAN OR EQUAL TO 12 YEARS, THE GUESTS AGE IS %d YEARS\n",allGuests[j].age);
          printf("\n");
          ...
          printf("NOT CHECKING THIS RIGHT NOW\n");
        ...
        if(allGuests[j].allDayRidePassOnGuest==1){
          ...
          printf("\n");
          printf("TESTING GUEST TO ATTEMPT TO GO ON RIDE WHEN THEIR AGE IS GREATER THAN 12 YEARS, THE GUESTS AGE IS %d YEARS\n",allGuests[j].age);
          printf("\n");
...
int calculateAgeValueOfGuest(int indexValueInGuestArray){
  ...
  int admitGuestToFair(Fair* theFair,Guest* theGuest){
    ...
    if(theGuest->age >= 13 && theGuest->totalMoneyOnGuest>=PRICE_OF_ADMISSION_ABOVE_13){
      ...
      printf("SUCCESSFULLY ADMITTED GUEST WHOSE AGE RANGES >= 13 \n");
      ...
      printf("SUCCESSFULLY ADMITTED GUEST WHOSE AGE RANGES > 6 AND < 13 \n");
      ...
      printf("SUCCESSFULLY ADMITTED GUEST WHOSE AGE RANGES < 6\n");
  ...
  int purchaseAllDayRidePassForGuest(Fair* theFair,Guest* theGuest){
    ...
    if(currentNumberOfAllDayRidePassesAvailable==0){
      ...
      if(theGuest->totalMoneyOnGuest >= COST_OF_ALL_DAY_RIDE_PASS){
        ...
        //theGuest->totalNumberOfTicketsOnGuest=0;//BECAUSE THEY JUST PURCHASED AN ALL DAY RIDE PASS
        ...
        printf("THE CURRENT AMOUNT OF MONEY ON THE GUEST IS $%.2f\n",theGuest->totalMoneyOnGuest);
        printf("THE ALL DAY RIDE PASS VALUE OF THE GUEST IS NOW %d\n",theGuest->allDayRidePassOnGuest);
        printf("ALSO THE AGE OF THE GUEST THAT BOUGHT THE ALL DAY RIDE PASS IS %d ",theGuest->age);
        printf("FINALLY THE TOTAL NUMBER OF RIDE TICKETS THE GUEST HAS IS %d TICKETS\n",theGuest->totalNumberOfTicketsOnGuest);
        ...
        theGuest->allDayRidePassOnGuest=0;
        printf("THE ALL DAY RIDE PASS VALUE OF THE GUEST AFTER NOT BEING ABLE TO GET ALL DAY RIDE PASS IS NOW %d\n",theGuest->allDayRidePassOnGuest);
  ...
  int guestAttemptsToGoOnRide(Guest* theGuest, Ride* theRide){
    ...
    printf("THE RIDE PASS VALUE OF THE GUEST IS %d \n",theGuest->allDayRidePassOnGuest);

    if(theGuest->heightInCm >= theRide->heightRequirementInCm){

      printf("THE GUEST SUCCESFULLY PASSED THE HEIGHT REQUIREMENT OF THE RIDE BECAUSE THEIR HEIGHT IS %d cm AND THE HEIGHT REQUIREMENT FOR THE RIDE IS %d cm\n",theGuest->heightInCm, theRide->heightRequirementInCm);

      if(theGuest->allDayRidePassOnGuest==1){

        printf("THE GUEST WAS ABLE TO GET ON THE RIDE BECAUSE THEIR ALL DAY RIDE PASS VALUE IS %d\n",theGuest->allDayRidePassOnGuest);

        return 1;

      }
      else if(theGuest->allDayRidePassOnGuest==0){

        printf("THE GUEST WAS NOT ABLE TO GET ON THE RIDE BECAUSE THEIR ALL DAY RIDE PASS VALUE IS %d\n",theGuest->allDayRidePassOnGuest);
