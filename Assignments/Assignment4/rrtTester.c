...
int main(int argc, char *argv[]) {
    ...
    /*if((unsigned char)*argv[0]>=5 && (unsigned char)*argv[0]<=100){           //Handling the first value in the arguments to set growthAmount value

      environment.growthAmount = (unsigned char)*argv[0];

    }

    if((unsigned short)*argv[1]>=5 && (unsigned short)*argv[1]<=5000){          //Handling the second value in the arguments to set maximumNodes value

      environment.maximumNodes = (unsigned short)*argv[1];

    }

    if((int)*argv[2]>=1 && (int)*argv[2]<=5){                                   //Handling the third value in the arguments to set environmentNumber value

      environmentNumber = (int)*argv[2];

    }
    else{

      break;                                                                    //Break when the values are not in the boundaries specified
